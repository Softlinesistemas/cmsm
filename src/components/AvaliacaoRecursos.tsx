...
const AvaliacaoRecursos = () => {
  ...
  const aprovarRecurso = async (numeroInscricao: number) => {
    ...
    try {
      await api.put(`api/candidato/${numeroInscricao}`, {
        isencao: "Deferido",
      });
      toast.success(
        `Pedido de insenção Aprovado para o candidato ${numeroInscricao}`
      );
      refetch();
    } catch (error: any) {
      toast.error(error.response.data.error || error.response.data.message);
  ...
  const desaprovarRecurso = async (numeroInscricao: number) => {
    ...
    try {
      ...
      toast.success(
        `Pedido de insenção Indeferido para o candidato ${numeroInscricao}`
      );
      refetch();
    } catch (error: any) {
      toast.error(error.response.data.error || error.response.data.message);
    ...
    <div className="p-4 bg-blue-800 rounded-xl shadow-xl shadow-gray-400">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border rounded-lg o  verflow-hidden">
          ...
          <tbody>
            {candidatosRecursos?.length ? (
              candidatosRecursos?.map((r: any) => (
                <tr
                  key={r.CodIns}
                  className="bg-blue-50 hover:bg-blue-100 text-black"
                >
                  ...
                  <td className="border px-4 py-2 flex flex-wrap gap-2 justify-center">
                    ...
                    >
                      {loadingReprovar ? <LoadingIcon /> : <FaTimes />}{" "}
                      Indeferir
                    </button>
                    <button
              ...
              <tr aria-colspan={6}>
                <p>Ainda não há recursos para análise.</p>
              </tr>
            )}
          </tbody>
        </table>
